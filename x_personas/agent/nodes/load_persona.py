...
def load_persona(state: PersonaState) -> dict:
    ...
    if path.stat().st_size == 0:
        raise ValueError(
            ...
            "Please copy the 14-section template from 'personas/_template/persona.md' "
    ...
    # Resolve source data files relative to the persona directory
    persona_dir = path.parent
    raw_source_files = _parse_source_data_files(lines)
    resolved_source_files = []
    for sf in raw_source_files:
        sf_path = Path(sf)
        if sf_path.is_absolute():
            resolved_source_files.append(sf)
        else:
            resolved_source_files.append(str(persona_dir / sf))

    sections["13"] = {"source_files": resolved_source_files}

    return {
        "persona_sections": sections,
        "source_data_files": resolved_source_files,
    }
