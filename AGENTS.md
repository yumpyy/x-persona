...
- Persona generator (`x_personas/generate_persona.py`) is separate from the agent — source data feeds into LLM → structured `persona-struct.md` → agent consumes it
...
- Prompts stored in `x_personas/prompts/` as markdown templates with placeholders
...
- Prompts in `x_personas/prompts/` — `llm_decide_system.md` (with `{persona_sections}` placeholder), `llm_decide_user.md` (with `{feed_posts}` placeholder)
- Pydantic models in `x_personas/models/engagement.py` — `PostDecision(action_type: list[str], reason, score)`, `EngagementDecisions(decisions: list[PostDecision])`
...
- New files: `x_personas/prompts/`, `x_personas/agent/nodes/llm_decide.py`
- Rewritten files: `graph.py` (llm_decide + generate_content + conditional routing), `runner.py` (dry-run calls LLM)
- Removed: entire `x_personas/agent/scoring/` directory, `score_posts.py`, `decide_engagement.py`
...
## Relevant Files
- `x_personas/prompts/llm_decide_system.md` — system prompt template for engagement decisions
- `x_personas/prompts/llm_decide_user.md` — user prompt template with feed posts
- `x_personas/agent/nodes/llm_decide.py` — LLM-driven decision node
- `x_personas/agent/nodes/generate_content.py` — dedicated LLM call per reply/quote with writing samples
- `x_personas/agent/nodes/execute_actions.py` — batch actions by post ID, one tab per post, 3-8s delays
- `x_personas/agent/graph.py` — perpetual loop with conditional routing: scroll_feed → llm_decide → [generate_content → execute_actions | execute_actions | log_activity] → log_activity → follow_decision → state_cleansing → scroll_page → END
- `x_personas/agent/runner.py` — `--visible`, `--quiet`, `--browser`, `--scroll-limit 2500`, perpetual loop with breaks, dry-run calls LLM
- `x_personas/agent/history.py` — `load_engaged_status_ids()` reads `<persona>-activity-log.md`
- `x_personas/agent/log.py` — `log()` and `set_quiet()`
- `x_personas/models/engagement.py` — `PostDecision`, `EngagementDecisions` Pydantic models for structured output
- `x_personas/utils/feed.py` — `scroll_down()` with smooth scroll
