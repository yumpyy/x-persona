...
async def main():
    if len(sys.argv) < 3:
        print("Usage: uv run python tests/test_post.py <handle> <status_id>")
        print("  e.g. uv run python tests/test_post.py imnot_linear 2060820483797332132")
        sys.exit(1)
    ...
    async with BrowserManager(headless=False) as bm:
        ...
        post = await get_post_data(page, status_id)

        print(f"\nStatus:    https://x.com/i/status/{post.status_id}")
